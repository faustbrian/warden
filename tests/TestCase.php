...
abstract class TestCase extends BaseTestCase
{
    ...
    protected function setUp(): void
    {
        ...
        self::registerVariableKeysForTests();
    ...
    public static function registerVariableKeysForTests(): void
    {
        $container = Container::getInstance();

        if (!$container->bound(VariableKeysRegistry::class)) {
            $container->singleton(VariableKeysRegistry::class, fn (): VariableKeysRegistry => new VariableKeysRegistry());
        }
    ...
    protected function defineEnvironment($app): void
    {
        ...
        $app['config']->set('warden.actor_morph_type', env('WARDEN_ACTOR_MORPH_TYPE', 'string'));
        $app['config']->set('warden.boundary_morph_type', env('WARDEN_BOUNDARY_MORPH_TYPE', 'string'));
        $app['config']->set('warden.subject_morph_type', env('WARDEN_SUBJECT_MORPH_TYPE', 'string'));

        // Register middleware aliases
        $app['router']->aliasMiddleware('role', HasRole::class);
        $app['router']->aliasMiddleware('permission', HasPermission::class);
        $app['router']->aliasMiddleware('role_or_permission', HasRoleOrPermission::class);
    }
