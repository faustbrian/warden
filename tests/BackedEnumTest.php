...
describe('Backed Enum Support', function (): void {
    describe('Happy Paths', function (): void {
        ...
        test('grants ability using backed enum', function ($provider): void {
            ...
            // Assert
            expect($warden->can(AbilityEnum::Create))->toBeTrue();
        })->with('bouncerProvider');
        ...
        test('grants multiple abilities using backed enums', function ($provider): void {
            ...
            // Assert
            expect($warden->can(AbilityEnum::Create))->toBeTrue();
            expect($warden->can(AbilityEnum::Update))->toBeTrue();
            expect($warden->can(AbilityEnum::Delete))->toBeTrue();
        })->with('bouncerProvider');

        test('checks multiple abilities using backed enums in canAny()', function ($provider): void {
            // Arrange
            [$warden, $user] = $provider();

            // Act
            $warden->allow($user)->to([AbilityEnum::Create, AbilityEnum::Delete]);

            // Assert
            expect($warden->canAny([AbilityEnum::Create, AbilityEnum::Update]))->toBeTrue();
            expect($warden->canAny([AbilityEnum::Read, AbilityEnum::Update]))->toBeFalse();
        })->with('bouncerProvider');

        test('checks abilities using backed enums via model can/cannot', function ($provider): void {
            // Arrange
            [$warden, $user] = $provider();
            $warden->allow($user)->to([AbilityEnum::Create, AbilityEnum::Delete]);

            // Assert
            expect($user->can(AbilityEnum::Create))->toBeTrue();
            expect($user->cannot(AbilityEnum::Delete))->toBeFalse();
            expect($user->cannot(AbilityEnum::Read))->toBeTrue();
        })->with('bouncerProvider');

        test('checks role using backed enum via direct model helper', function ($provider): void {
            // Arrange
            [$warden, $user] = $provider();
            $warden->assign(RoleEnum::Admin)->to($user);

            // Assert
            expect($user->isAn(RoleEnum::Admin))->toBeTrue();
            expect($user->isNotAn(RoleEnum::Admin))->toBeFalse();
            expect($user->isA(RoleEnum::Admin))->toBeTrue();
            expect($user->isNotA(RoleEnum::Admin))->toBeFalse();
            expect($user->isAll(RoleEnum::Admin))->toBeTrue();
        })->with('bouncerProvider');
