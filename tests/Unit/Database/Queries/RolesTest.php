...
describe('Roles Query', function (): void {
    ...
    describe('Happy Paths', function (): void {
        ...
        test('constrain where is filters by single backed enum role', function (): void {
            // Arrange
            Role::query()->create(['name' => RoleEnum::Admin->value]);

            $user1 = User::query()->create(['name' => 'Admin User']);
            $warden = $this->bouncer($user1);
            $warden->assign(RoleEnum::Admin->value)->to($user1);

            // Act
            $query = User::query();
            $this->rolesQuery->constrainWhereIs($query, RoleEnum::Admin);
            $users = $query->get();
    ...
    describe('Regression Tests - Keymap Support', function (): void {
        ...
        test('constrainWhereAssignedTo with class and keys uses keymap values', function (): void {
            ...
            $query = Role::query();
            $this->rolesQuery->constrainWhereAssignedTo($query, User::class, [$user2->id]); // Bob's keymap value
            $roles = $query->get();
