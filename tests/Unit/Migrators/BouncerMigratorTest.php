...
beforeEach(function (): void {
    ...
    // Add migrated_at columns to bouncer tables for migration tracking tests
    $connection = $this->app['config']->get('database.default');

    if (Schema::connection($connection)->hasTable('bouncer_roles') && !Schema::connection($connection)->hasColumn('bouncer_roles', 'migrated_at')) {
        Schema::connection($connection)->table('bouncer_roles', function (Blueprint $table): void {
            $table->timestamp('migrated_at')->nullable();
        });
    }

    if (Schema::connection($connection)->hasTable('bouncer_abilities') && !Schema::connection($connection)->hasColumn('bouncer_abilities', 'migrated_at')) {
        Schema::connection($connection)->table('bouncer_abilities', function (Blueprint $table): void {
            $table->timestamp('migrated_at')->nullable();
        });
    }

    if (Schema::connection($connection)->hasTable('bouncer_assigned_roles') && !Schema::connection($connection)->hasColumn('bouncer_assigned_roles', 'migrated_at')) {
        Schema::connection($connection)->table('bouncer_assigned_roles', function (Blueprint $table): void {
            $table->timestamp('migrated_at')->nullable();
        });
    }

    if (Schema::connection($connection)->hasTable('bouncer_permissions') && !Schema::connection($connection)->hasColumn('bouncer_permissions', 'migrated_at')) {
        Schema::connection($connection)->table('bouncer_permissions', function (Blueprint $table): void {
            $table->timestamp('migrated_at')->nullable();
        });
    }

    if (!Schema::connection($connection)->hasTable('bouncer_roles') || Schema::connection($connection)->hasColumn('bouncer_roles', 'last_migrated')) {
        return;
    }
...
describe('BouncerMigrator', function (): void {
    ...
    describe('Idempotent Migrations - migrated_at Tracking', function (): void {
        test('marks migrated roles with migrated_at timestamp when tracking enabled', function (): void {
            // Arrange
            $migrationTracking = ['enabled' => true, 'column' => 'migrated_at'];
            ...
            $migrator = new BouncerMigrator(BouncerUser::class, migrationTracking: $migrationTracking);
        ...
        test('marks migrated abilities with migrated_at timestamp when tracking enabled', function (): void {
            // Arrange
            $migrationTracking = ['enabled' => true, 'column' => 'migrated_at'];
            ...
            $editPostsAbilityId = DB::table('bouncer_abilities')->insertGetId([
                'name' => 'edit-posts',
                'title' => 'Edit Posts',
                'entity_id' => null,
                'entity_type' => null,
            ...
            DB::table('bouncer_permissions')->insert([
                ['ability_id' => $editPostsAbilityId, 'entity_id' => $roleId, 'entity_type' => null, 'forbidden' => false, 'scope' => null],
                ['ability_id' => $deletePostsAbilityId, 'entity_id' => $roleId, 'entity_type' => null, 'forbidden' => false, 'scope' => null],
            ]);
            ...
            // Assert - Abilities migrated and marked with timestamp
            $this->assertDatabaseHas('bouncer_abilities', [
                'name' => 'edit-posts',
                'migrated_at' => now()->toDateTimeString(),
            ]);
            $this->assertDatabaseHas('bouncer_abilities', [
        ...
        test('skips already migrated roles on second run', function (): void {
            ...
            DB::table('bouncer_roles')->insert([
                ['name' => 'admin', 'title' => 'Administrator', 'scope' => null, 'migrated_at' => $firstMigrationTime, 'created_at' => now(), 'updated_at' => now()],
            ]);
        ...
        test('migrates new roles while skipping already migrated ones', function (): void {
            ...
            Models::role()->create(['name' => 'admin', 'title' => 'Administrator']);

            DB::table('bouncer_roles')->insert([
                ['name' => 'admin', 'title' => 'Administrator', 'scope' => null, 'migrated_at' => $firstMigrationTime, 'created_at' => now(), 'updated_at' => now()],
                ['name' => 'editor', 'title' => 'Editor', 'scope' => null, 'migrated_at' => null, 'created_at' => now(), 'updated_at' => now()],
            ]);
        ...
        test('handles tracking with custom column name', function (): void {
            // Arrange - Tracking enabled with custom column name
            $migrationTracking = ['enabled' => true, 'column' => 'last_migrated'];
            ...
            // Assert - Role migrated and marked with custom column
            $this->assertDatabaseHas('roles', ['name' => 'admin']);
            $role = DB::table('bouncer_roles')->where('name', 'admin')->first();
        ...
        test('uses custom migrated_at column name from config', function (): void {
            ...
            DB::table('bouncer_roles')->insert([
                ['name' => 'viewer', 'title' => 'Viewer', 'scope' => null, 'created_at' => now(), 'updated_at' => now()],
            ]);
            ...
            $role = DB::table('bouncer_roles')->where('name', 'viewer')->first();
            expect($role->last_migrated)->not->toBeNull();
        });

        test('does not track when tracking disabled in config', function (): void {
            // Arrange
            $migrationTracking = ['enabled' => false, 'column' => 'migrated_at'];
