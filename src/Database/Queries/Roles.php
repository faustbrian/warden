...
final class Roles
{
     ...
     * @param BackedEnum|string ...$roles One or more role names to filter by
     *
     * @return Builder<TModel> The constrained query
     */
    public function constrainWhereIs(Builder $query, BackedEnum|string ...$roles): Builder
    {
        $normalized = array_map(Helpers::normalizeEnumValue(...), $roles);

        return $query->whereHas('roles', function (Builder $query) use ($normalized): void {
            $query->whereIn('name', $normalized);
        });
    }
     ...
     *
     * @param Builder<TModel>   $query    The query builder instance
     * @param BackedEnum|string ...$roles One or more role names that must all be present
     *
     * @return Builder<TModel> The constrained query
     */
    public function constrainWhereIsAll(Builder $query, BackedEnum|string ...$roles): Builder
    {
        $normalized = array_map(Helpers::normalizeEnumValue(...), $roles);
     ...
     *
     * @param Builder<TModel>   $query    The query builder instance
     * @param BackedEnum|string ...$roles One or more role names to exclude
     *
     * @return Builder<TModel> The constrained query
     */
    public function constrainWhereIsNot(Builder $query, BackedEnum|string ...$roles): Builder
    {
        $normalized = array_map(Helpers::normalizeEnumValue(...), $roles);

        return $query->whereDoesntHave('roles', function (Builder $query) use ($normalized): void {
            $query->whereIn('name', $normalized);
        });
    }
