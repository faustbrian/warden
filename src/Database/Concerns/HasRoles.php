...
trait HasRoles
{
     ...
     *
     * @param BackedEnum|string ...$roles One or more role names to check
     *
     * @return bool True if the model has any of the specified roles
     */
    public function isAn(BackedEnum|string ...$roles): bool
    {
     ...
     * @return bool True if the model has any of the specified roles
     */
    public function isA(BackedEnum|string ...$roles): bool
    {
     ...
     */
    public function isNotAn(BackedEnum|string ...$roles): bool
    {
        /** @var array<int, int|Role|string> $normalized */
        $normalized = array_map(Helpers::normalizeEnumValue(...), array_values($roles));
     ...
     * @return bool True if the model has none of the specified roles
     */
    public function isNotA(BackedEnum|string ...$roles): bool
    {
     ...
     */
    public function isAll(BackedEnum|string ...$roles): bool
    {
        /** @var array<int, int|Role|string> $normalized */
        $normalized = array_map(Helpers::normalizeEnumValue(...), array_values($roles));
     ...
     * @param BackedEnum|string ...$role One or more role names to filter by
     */
    protected function scopeWhereIs(Builder $query, BackedEnum|string ...$role): void
    {
        $normalized = array_map(Helpers::normalizeEnumValue(...), array_values($role));
        new RolesQuery()->constrainWhereIs($query, ...$normalized);
    }
     ...
     *
     * @param Builder<static>   $query   The query builder instance
     * @param BackedEnum|string ...$role One or more role names that must all be present
     */
    protected function scopeWhereIsAll(Builder $query, BackedEnum|string ...$role): void
    {
        $normalized = array_map(Helpers::normalizeEnumValue(...), array_values($role));
        new RolesQuery()->constrainWhereIsAll($query, ...$normalized);
    }
     ...
     *
     * @param Builder<static>   $query   The query builder instance
     * @param BackedEnum|string ...$role One or more role names to exclude
     */
    protected function scopeWhereIsNot(Builder $query, BackedEnum|string ...$role): void
    {
        $normalized = array_map(Helpers::normalizeEnumValue(...), array_values($role));
        new RolesQuery()->constrainWhereIsNot($query, ...$normalized);
    }
}
