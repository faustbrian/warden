...
trait IsRole
{
     ...
     * Optionally filters by guard name for multi-guard applications.
     *
     * @param  iterable<string> $names     Role names to look up
     * @param  null|string      $guardName Optional guard name to filter by
     * @return array<int>       Array of matching role IDs
     */
    public function getKeysByName($names, ?string $guardName = null): array
    {
        if (empty($names)) {
            return [];
        }
        ...
        /** @var array<int> */
        return $query->select($this->getKeyName())->get()
     ...
     */
    public function getNamesByKey($keys, ?string $guardName = null): array
    {
        if (empty($keys)) {
            return [];
        }

        $query = $this->whereIn($this->getKeyName(), $keys);
    ...
    /**
     * Query scope to filter roles by guard name.
     *
     * Constrains the roles query to only include roles for a specific guard.
     * Useful for multi-guard applications where roles are segregated by authentication guard.
     *
    ...
    protected function createAssignRecords(Model $model, array $keys): array
    {
        ...
        return array_map(function (int $key) use ($type, $primaryKeyType): array {
            ...
            if ($primaryKeyType !== PrimaryKeyType::ID) {
                $record['id'] = match ($primaryKeyType) {
