...
use Cline\Warden\Exceptions\GateNotConfiguredException;
use Cline\Warden\Exceptions\ModelClassDoesNotExistException;
...
final class Warden
{
     ...
     *
     * @throws GateNotConfiguredException If gate instance has not been set
     *
     * @return Gate The configured gate instance
     */
    public function gate(): Gate
    {
        throw_if(!$this->gate instanceof Gate, GateNotConfiguredException::notSet());
     ...
     *
     * @param BackedEnum|string $ability   The ability to authorize
    ...
    {
        $ability = $this->normalizeAbilityForGate($ability);
     ...
     *
     * @param BackedEnum|string $ability   The ability to check
     * @param array|mixed       $arguments Additional arguments for the authorization check
     *
     * @return bool True if allowed, false otherwise
     */
    public function can(BackedEnum|string $ability, mixed $arguments = []): bool
    {
        $ability = $this->normalizeAbilityForGate($ability);
     ...
     *
     * @param array<int, BackedEnum|string> $abilities Array of ability names to check
    ...
    {
        $normalizedAbilities = array_map(Helpers::normalizeEnumValue(...), $abilities);

        return $this->gate()->any($normalizedAbilities, $arguments);
    }
     ...
     */
    public function cannot(BackedEnum|string $ability, mixed $arguments = []): bool
    {
        $ability = $this->normalizeAbilityForGate($ability);
    ...
    public function allows(BackedEnum|string $ability, mixed $arguments = []): bool
    {
    ...
    public function denies(BackedEnum|string $ability, mixed $arguments = []): bool
    {
     ...
     *
     * @throws ModelClassDoesNotExistException If the class does not exist
     *
     * @return $this
     */
    public function useAbilityModel(string $model): self
    {
        throw_unless(class_exists($model), ModelClassDoesNotExistException::forClass($model));
    ...
    /**
     * Normalize ability values for Gate method compatibility.
     *
     * Converts BackedEnum instances to their backing value while preserving
     * non-enum string values. This ensures the type remains compatible with
     * Laravel Gate's expected signatures.
     */
    private function normalizeAbilityForGate(BackedEnum|string $ability): string|BackedEnum
    {
        $ability = Helpers::normalizeEnumValue($ability);

        assert(is_string($ability) || $ability instanceof BackedEnum);
