...
use Cline\Warden\Exceptions\InvalidGateSlotException;
...
final class Guard
{
     ...
     *
     * @throws InvalidGateSlotException If slot value is not 'before' or 'after'
     *
    ...
    public function slot(?string $slot = null): string|self
    {
        ...
        throw_unless(in_array($slot, ['before', 'after'], true), InvalidGateSlotException::invalidSlot($slot));
    ...
    {
        $gate->before(function (Model $authority, BackedEnum|string $ability, array $arguments = []): mixed {
        ...
        $gate->after(function (Model $authority, BackedEnum|string $ability, mixed $result, array $arguments = []): mixed {
            // @phpstan-ignore-next-line function.alreadyNarrowedType,instanceof.alwaysTrue - Runtime safety check
            assert($authority instanceof Model);
     ...
     */
    private function runBeforeCallback(Model $authority, BackedEnum|string $ability, array $arguments = []): mixed
    {
        if ($this->slot !== 'before') {
            return null;
        }
     ...
     */
    private function runAfterCallback(Model $authority, BackedEnum|string $ability, mixed $result, array $arguments = []): mixed
    {
        if (null !== $result) {
            return $result;
        }
     ...
     */
    private function checkAtClipboard(Model $authority, BackedEnum|string $ability, Model|string|null $model): mixed
    {
