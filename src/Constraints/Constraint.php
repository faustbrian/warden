...
use Cline\Warden\Exceptions\ColumnNameMustBeStringException;
use Cline\Warden\Exceptions\InvalidComparisonOperatorException;
use Cline\Warden\Exceptions\LogicalOperatorMustBeStringException;
use Cline\Warden\Exceptions\OperatorMustBeStringException;
...
abstract class Constraint implements Constrainer
{
    ...
    public static function whereColumn(string $a, mixed $operator, mixed $b = null): ColumnConstraint
    {
        ...
        throw_unless(is_string($b), ColumnNameMustBeStringException::forWhereColumn());
    ...
    public function logicalOperator(mixed $operator = null): static|string
    {
        ...
        if (!is_string($operator)) {
            throw LogicalOperatorMustBeStringException::fromValue($operator);
        }
    ...
    protected static function prepareOperatorAndValue(mixed $operator, mixed $value, mixed $usesDefault): array
    {
        ...
        throw_unless(is_string($operator), OperatorMustBeStringException::forConstraint());

        throw_unless(in_array($operator, ['=', '==', '!=', '<', '>', '<=', '>='], true), InvalidComparisonOperatorException::unsupported($operator));
    ...
    protected function compare(mixed $a, mixed $b): bool
    {
        return match ($this->operator) {
            ...
            '<=' => $a <= $b,
            '>=' => $a >= $b,
            default => throw InvalidComparisonOperatorException::unsupported($this->operator),
        };
    }
}
