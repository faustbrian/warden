 ...
 * @see     CachedClipboard Cached alternative for performance
 *
...
final class Clipboard extends AbstractClipboard
{
     ...
     */
    public function checkGetId(Model $authority, BackedEnum|string $ability, Model|string|null $model = null): bool|int|string|null
    {
     ...
     */
    private function isForbidden(Model $authority, BackedEnum|string $ability, Model|string|null $model = null): bool
    {
     ...
     */
    private function getAllowingAbility(Model $authority, BackedEnum|string $ability, Model|string|null $model = null): ?Ability
    {
        /** @var null|Ability */
        return $this->getHasAbilityQuery(
            $authority,
     ...
     */
    private function getHasAbilityQuery(Model $authority, BackedEnum|string $ability, Model|string|null $model, bool $allowed): Builder
    {
        ...
        // Normalize BackedEnum to string value for byName scope
        /** @var string $abilityName */
        $abilityName = Helpers::normalizeEnumValue($ability);
        $query->byName($abilityName);
     ...
     */
    private function constrainToSimpleAbility(Builder $query, BackedEnum|string $ability): Builder
    {
        $abilityName = Helpers::normalizeEnumValue($ability);

        return $query->where(function (Builder $query) use ($abilityName): void {
            $query->where('name', $abilityName)->whereNull('subject_type');
