 ...
 *
 * @see     AbstractClipboard Base clipboard implementation
 * @see     Clipboard Non-cached alternative
 *
...
final class CachedClipboard extends AbstractClipboard implements CachedClipboardInterface
{
     ...
     *
     * @param Store $cache The cache store instance to use for caching authorization data.
     *                     Tagged caching will be enabled automatically if the driver supports
     *                     it (e.g., Redis, Memcached), enabling efficient bulk cache invalidation.
     *                     Drivers without tag support (file, database) will use individual keys.
     */
     ...
     * @return null|false|int|string False if forbidden, ability ID if allowed, null if not found
     */
    public function checkGetId(Model $authority, BackedEnum|string $ability, Model|string|null $model = null): false|int|string|null
    {
     ...
     * @return null|int|string                                   The matched ability ID, or null if no match found
     */
    ...
    {
        /** @var BaseCollection<int|string, string> */
     ...
     *
     * @param  BaseCollection<TKey, string> $abilityMap Map of ability IDs to their identifiers
     ...
     *
     * @param  BackedEnum|string           $ability The ability name being checked
     ...
     */
    private function compileAbilityIdentifiers(BackedEnum|string $ability, Model|string|null $model): BaseCollection
    {
        $abilityString = $this->normalizeAbility($ability);

        $identifiers = new BaseCollection(
            null === $model
                ? [$abilityString, '*-*', '*']
                : $this->compileModelAbilityIdentifiers($abilityString, $model),
        );

        /** @var BaseCollection<int, string> */
        // @codeCoverageIgnoreStart
        return $identifiers->map(fn (string $value): string => mb_strtolower($value));
    ...
    /**
     * Normalize a BackedEnum or string ability to a string.
     *
     * Converts BackedEnum instances to their backing value string, while
     * passing through plain strings unchanged. Ensures consistent string
     * handling throughout the authorization system.
     *
     * @param  BackedEnum|string $ability The ability to normalize
     * @return string            The normalized ability string
     */
    private function normalizeAbility(BackedEnum|string $ability): string
    {
        return $ability instanceof BackedEnum ? (string) $ability->value : $ability;
    }
}
