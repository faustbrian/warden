...
final class WardenServiceProvider extends PackageServiceProvider
{
    ...
    {
        $this->app->singleton(Warden::class, function (): Warden {
    ...
    /**
     * Register Warden models with VariableKeys for primary key configuration.
     *
     * Maps Warden's core models (Role, Permission, Ability, AssignedRole) to use
     * the configured primary key type from warden.primary_key_type. This enables
     * the use of auto-increment IDs, ULIDs, or UUIDs as primary keys across all
     * Warden models.
     */
    ...
    /**
     * Register Blade authorization conditionals for role checks.
     *
     * Registers a `role` conditional via Blade::if() so consumers can use:
     * - @role(...)
     * - @elserole(...)
     * - @unlessrole(...)
     * - @endrole
    ...
    private function registerBladeConditionals(): void
    {
        Blade::if('role', function (string ...$roles): bool {
            ...
            return WardenFacade::is($user)->a(...$roles);
        });
    }
